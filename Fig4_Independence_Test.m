% Perform independence test in neurons with significant auROC 
% (grip-force and grip-only neurons).
% feb 2025, @apms
%

clear,clc,close all
load('independencePermutations.mat')
load('ROC_data.mat')
load('eventTimes')
...
for m = 1:length(monkeys)
    ...
    xlabel('Time (s)'), ylabel('abs(auROC)')

    % mark significantly dependent bins
    onlygrip_dependent_bins = logical(independencePermutations.(monkey).onlygrip_units.dependent_bins);
    ...
    % Plot marginal distributions and 95%-confidence ellipse
    subplot(233)
    x = force_ROC(onlygrip_units,ot_timebin);
    y = grip_ROC(onlygrip_units,ot_timebin);
    pValue = independencePermutations.(monkey).onlygrip_units.pValues(ot_timebin);
    
    GMModel = fitgmdist([x, y], 1);
    mu = GMModel.mu; Sigma = GMModel.Sigma;

    plot(x, y, '.'), hold on
    angle = error_ellipse(Sigma, mu, 'conf', 0.95);
    text(0.2,0.8,num2str(angle)), hold on
    text(0.8,0.8,num2str(pValue)), hold on
    line([0.5 0.5],[0 1], 'linestyle','--','color','k'), hold on
    line([0 1],[0.5 0.5], 'linestyle','--','color','k'), hold on
    xlim([0 1]), ylim([0 1]), 
    ...
    % mark significantly dependent bins
    gripforce_dependent_bins = logical(independencePermutations.(monkey).gripforce_units.dependent_bins);
    ...
    % Plot marginal distributions and 95%-confidence ellipse
    subplot(236)
    x = force_ROC(gripforce_units,ot_timebin);
    y = grip_ROC(gripforce_units,ot_timebin);
    pValue = independencePermutations.(monkey).gripforce_units.pValues(ot_timebin);
